...
def plot_data(axes, x, y, pred, exact, x_range, y_range):
    ...
    if exact != None:
        ...
        img3 = axes[2].imshow(pred-exact,extent = (x_range[0],x_range[1],y_range[0],y_range[1]), aspect = "auto", vmin = -0.045, vmax = 0.045, cmap = cmap)
...
def plot(model, model_params, target, target_params, x_range, y_range, filename):
    fig, axes = plt.subplots(3, 10, squeeze=False,
                             constrained_layout=False, figsize=(2*11+0.5, 2.5*3))
    ...
    axes[0,6].set_title('Well 7\nCumulative', fontsize=16)
    axes[0,7].set_title('Well 8\nCumulative', fontsize=16)
    axes[0,8].set_title('Well 9\nCumulative', fontsize=16)
    axes[0,9].set_title('Well 10\nCumulative', fontsize=16)
...
distribution = Q_Tensor_Product([distribution] * 10)



model, model_params, target, target_params = get_model_and_params(
    distribution,
    "flow_params/Q/CNF_10_Num_FFJORD_Adaptive_layer_size=80_network_layers=5_KL_model_control_num_samples=500_epochs=8000_cosine_decay_warmup=0.1.flow",
)
...
plot(
    ...
    x_range = (-5,5,0.1),
    y_range = (-5,5,0.1),
    filename = "figure_plotting/figure5.pdf",
)