...
class KL_sample_model_reparam(Loss):
    def __init__(self, 
                 rng,
                 model, 
                 target, 
    ...
    def loss(self, model_params, target_params, rng, sampler_state, validation = False):
        ...
        return (ratio * log_ratio).mean(), sampler_state
...
class KL_sample_model_control_variance(Loss):
    def __init__(self, 
                 rng,
                 model, 
                 target, 
    ...
    def loss(self, model_params, inputs, target_logprobs, rng):
        ...
        return (ratio * log_ratio).mean()
    
    def loss_grad(self, model_params, inputs, target_logprobs, rng):
        model_logprobs = self.model.log_Q(model_params, inputs, rng)
            
        ratio = -jnp.exp(target_logprobs - model_logprobs)

        return (stop_gradient(ratio - ratio.mean()) * model_logprobs).mean()
    ...
    def __call__(self, rng, model_params, target_params, sampler_state = None, validation = False):
        ...
        L1 = eqx.filter_jit(self.loss)(model_params, inputs, target_logprobs, key3)
...
class KL_sample_uniform(Loss):
    ...
    def __init__(self, 
                 ...
                 has_validation = False,
                 grad_loss = False,
                 name = "KL_sample_uniform",
                 control_variance = "mean",
                 **kwargs):
    ...
    @eqx.filter_jit
    def loss(self, model_params):
        model_log_Q = self.model.log_Q(model_params, self.samples, None)
            
        return (jnp.exp(self.target_log_Q) * (self.target_log_Q - model_log_Q)).sum() * self.scale
    
    @eqx.filter_jit
    def loss_grad(self, model_params):
        model_log_Q = self.model.log_Q(model_params, self.samples, None)

        if self.control_variance == "mean":
            print("CONTROL VARIANCE = mean")
            cv = jnp.mean(jnp.exp(self.target_log_Q - model_log_Q))*jnp.exp(model_log_Q)
        elif self.control_variance == "1":
            print("CONTROL VARIANCE = 1")
            cv = jnp.exp(model_log_Q)
        elif self.control_variance == "none":
            print("CONTROL VARIANCE = none")
            cv = 0
            
        return -(stop_gradient(jnp.exp(self.target_log_Q) - cv)*model_log_Q).mean(axis = 0) * self.scale

    def __call__(self, rng, model_params, target_params, sampler_state = None, validation = False):
        KL = eqx.filter_jit(self.loss)(model_params)

        if self.grad_loss and not validation:
            KL_grad = eqx.filter_jit(eqx.filter_grad(self.loss_grad))(model_params)
            return (KL,KL_grad), sampler_state
        
        return KL, sampler_state
    
class KL_sample_efficient(Loss):
    control_variate: str
    ...
    def __init__(self, 
                 ...
                 has_validation = False,
                 grad_loss = False,
                 name = "KL_sample_efficient",
                 control_variate = "mean",
                 **kwargs):
    ...
    def loss(self, model_params, samples, sample_logprobs, target_logprobs, rng):
        model_logprobs = self.model.log_Q(model_params, samples, rng)

        log_target_over_sample = target_logprobs - sample_logprobs
        log_target_over_model = target_logprobs - model_logprobs

        target_over_sample = jnp.exp(log_target_over_sample)
            
        return (target_over_sample * log_target_over_model).mean()

    def loss_grad(self, model_params, samples, sample_logprobs, target_logprobs, rng):
        model_logprobs = self.model.log_Q(model_params, samples, rng)

        target_over_sample = jnp.exp(target_logprobs - sample_logprobs)
        model_over_sample = jnp.exp(model_logprobs - sample_logprobs)
        target_over_model = jnp.exp(target_logprobs - model_logprobs)

        if self.control_variate == "mean":
            return -(stop_gradient(target_over_sample - model_over_sample*jnp.mean(target_over_model)) * model_logprobs).mean()
        elif self.control_variate == "1":
            print("CONTROL VARIANCE = 1")
            return -(stop_gradient(target_over_sample - model_over_sample) * model_logprobs).mean()
        
        raise ValueError("Control variate must be either 'mean' or '1'")

    def __call__(self, rng, model_params, target_params, sampler_state = None, validation = False):
        key1, key2, key3 = split(rng, 3)

        samples, sample_logprobs, target_logprobs, sampler_state = self.sampler.sample(key1, model_params, target_params, sampler_state, validation = validation)

        KL = eqx.filter_jit(self.loss)(model_params, samples, sample_logprobs, target_logprobs, key2)

        if self.grad_loss and not validation:
            KL_grad = eqx.filter_jit(eqx.filter_grad(self.loss_grad))(model_params, samples, sample_logprobs, target_logprobs, key3)
            return (KL,KL_grad), sampler_state
